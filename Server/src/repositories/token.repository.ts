...
export class TokenRepository {
  public async saveToken(userId: number, refreshToken: string) {
    try {
      ...
      throw new HttpException(404, ExceptionType.DB_USER_TOKEN_GET_NOT_GOT);
    }
  }
  public async removeToken(refreshToken: string) {
    try {
      ...
      } else throw new HttpException(404, ExceptionType.DB_USER_TOKEN_GET_NOT_GOT);
    } catch (error) {
      throw new HttpException(404, ExceptionType.DB_USER_TOKEN_GET_NOT_GOT);
    }
  }
  public async findRefreshToken(refreshToken: string) {
    try {
      const token = await Token.findOne({ where: { refreshToken: refreshToken } });
      if (!token) throw new HttpException(404, ExceptionType.DB_USER_TOKEN_GET_NOT_GOT);
      return token;
    } catch (error) {
      throw new HttpException(404, ExceptionType.DB_USER_TOKEN_GET_NOT_GOT);
    }
